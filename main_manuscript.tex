...
A common criticism of ensemble-based oversampling methods is the increased computational overhead caused by repeatedly training base learners on minority instances. To evaluate whether Dynamic CS-ARF remains industrially feasible for real-time environments, we profiled its Transaction Per Second (TPS) throughput, CPU utilization, and Memory allocation against the Standard ARF on 50,000 transactions.

\begin{table}[htbp]
    \centering
    \caption{Computational profiling (Throughput, Memory Delta, and CPU utilization) on 50,000 continuous samples.}
    \label{tab:throughput}
    \begin{tabular}{llccc}
        \toprule
        \textbf{Dataset} & \textbf{Model} & \textbf{Throughput (TPS)} & \textbf{CPU (\%)} & \textbf{Memory Delta (MB)} \\
        \midrule
        \multirow{2}{*}{Agrawal} & ARF (Standard) & 684.92 & 99.6 & 10.90 \\
        & Dynamic CS-ARF & 445.39 & 99.1 & 0.10 \\
        \midrule
        \multirow{2}{*}{Hyperplane} & ARF (Standard) & 806.95 & 99.4 & 10.89 \\
        & Dynamic CS-ARF & 502.40 & 99.3 & 0.06 \\
        \bottomrule
    \end{tabular}
\end{table}

As shown in Table~\ref{tab:throughput}, the dynamic Poisson penalty undeniably imposes a computational cost, reducing the throughput from $\sim$685--806 TPS in Standard ARF to $\sim$445--502 TPS in Dynamic CS-ARF. Both models fully utilize a single CPU core ($\sim$99\%), while memory growth remains negligible due to the incremental nature of Hoeffding Trees. 

Despite this reduction, achieving $\sim$500 TPS on a single-threaded Python process indicates strong industrial feasibility. For context, major credit card networks like Visa process an average of 1,700 TPS globally. Because stream-based trees evaluate transactions independently, Dynamic CS-ARF can be straightforwardly parallelized across a modern 4- or 8-core CPU array to comfortably exceed global transaction velocities, making it highly viable for real-time industrial deployment.
